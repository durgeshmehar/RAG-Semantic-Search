...
from app.services.upload import validators


def test_plain_ascii_text_is_not_binary():
    sample = b"INFO Starting server\nERROR Connection failed\n" * 50
    assert not validators.looks_like_binary(sample)
...
def test_empty_sample_is_not_binary():
    assert not validators.looks_like_binary(b"")
...
def test_a_single_null_byte_is_binary():
    ...
    assert validators.looks_like_binary(sample)
...
def test_a_chunk_boundary_mid_multibyte_character_is_not_binary():
    ...
    truncated = whole[:-1]
    assert not validators.looks_like_binary(truncated)
...
def test_only_the_configured_sample_size_is_scanned():
    ...
    assert len(good_prefix) > validators.SAMPLE_BYTES
    garbage_suffix = b"\x00" * 1000
    assert not validators.looks_like_binary(good_prefix + garbage_suffix)
