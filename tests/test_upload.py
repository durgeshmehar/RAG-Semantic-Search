...
def test_create_upload_returns_file_id(client):
    response = client.post(f"{API}/files", json={"filename": "a.log", "total_size": 100})
...
def test_interrupted_upload_resumes_from_reported_offset(client, sample_text):
    ...
    response = client.post(
        f"{API}/files", json={"filename": "big.log", "total_size": len(sample_text)}
    )
    ...
    for _ in range(3):
        ...
        client.put(f"{API}/files/{file_id}/chunk", params={"offset": sent}, content=piece)
        sent += len(piece)
    ...
    while offset < len(sample_text):
        ...
        response = client.put(
            f"{API}/files/{file_id}/chunk", params={"offset": offset}, content=piece
        )
        assert response.status_code == 200
        offset += len(piece)

    client.post(f"{API}/files/{file_id}/complete")
...
def test_wrong_offset_is_rejected_with_resume_hint(client, sample_text):
    ...
    response = client.post(
        f"{API}/files", json={"filename": "x.log", "total_size": len(sample_text)}
    )
    file_id = response.json()["file_id"]

    client.put(f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=sample_text[:500])

    # Skipping ahead would leave a hole in the file.
    response = client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": 9999}, content=b"junk"
    )
    ...
    response = client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=sample_text[:500]
    )
    assert response.status_code == 409
...
def test_upload_beyond_max_file_bytes_is_rejected(client, monkeypatch):
    ...
    response = client.post(f"{API}/files", json={"filename": "s.log", "total_size": 10})
    file_id = response.json()["file_id"]

    response = client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=b"x" * 50
    )
...
def test_binary_first_chunk_is_rejected(client):
    ...
    response = client.post(
        f"{API}/files", json={"filename": "photo.png", "total_size": len(png_signature)}
    )
    file_id = response.json()["file_id"]

    response = client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=png_signature
    )
...
def test_binary_check_only_applies_to_the_first_chunk(client):
    ...
    response = client.post(
        f"{API}/files", json={"filename": "log.txt", "total_size": len(first) + len(second)}
    )
    file_id = response.json()["file_id"]

    r1 = client.put(f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=first)
    assert r1.status_code == 200

    r2 = client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": len(first)}, content=second
    )
...
def test_upload_may_exceed_its_own_declared_total_size(client):
    ...
    response = client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=b"x" * 20
    )
    assert response.status_code == 200
...
def test_completed_upload_rejects_more_chunks(client):
    ...
    response = client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": len(data)}, content=b"more"
    )
    assert response.status_code == 409


def test_complete_requires_at_least_one_chunk(client):
    response = client.post(f"{API}/files", json={"filename": "empty.log", "total_size": 10})
    file_id = response.json()["file_id"]
...
def test_declared_total_size_is_corrected_to_actual_bytes_on_complete(client):
    ...
    response = client.post(
        f"{API}/files", json={"filename": "s.log", "total_size": 999999}
    )
    file_id = response.json()["file_id"]
    data = b"only twelve b"
    client.put(f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=data)
    client.post(f"{API}/files/{file_id}/complete")
...
def test_passages_are_enqueued_during_upload(client, sample_text):
    ...
    response = client.post(
        f"{API}/files", json={"filename": "stream.log", "total_size": len(sample_text)}
    )
    file_id = response.json()["file_id"]
    ...
    client.put(
        f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=sample_text[:4000]
    )
...
def test_files_are_scoped_to_their_owner(client):
    ...
    assert client.get(f"{API}/files/{file_id}/status", headers=other).status_code == 404
    assert (
        client.put(
            f"{API}/files/{file_id}/chunk", params={"offset": 0}, content=b"x", headers=other
        ).status_code
        == 404
    )
    assert (
        client.post(f"{API}/files/{file_id}/search", json={"query": "x"}, headers=other).status_code
        == 404
    )
    assert client.delete(f"{API}/files/{file_id}", headers=other).status_code == 404

    # The owner can still do all of this.
    assert client.get(f"{API}/files/{file_id}/status").status_code == 200
...
def test_listing_only_shows_the_caller_s_own_files(client):
    ...
    theirs_resp = client.post(
        f"{API}/files", json={"filename": "theirs.log", "total_size": 5}, headers=other_headers
    )
    theirs = theirs_resp.json()["file_id"]

    my_listing = {f["file_id"] for f in client.get(f"{API}/files").json()["files"]}
    their_listing = {
        f["file_id"] for f in client.get(f"{API}/files", headers=other_headers).json()["files"]
    }
